// items ids
const STONE = 1;
const GRASS = 2;
const DIRT = 3;
const COBBLESTONE = 4;
const WOOD_PLANK = 5;
const SAPLING = 6;
const BEDROCK = 7;
const WATER = 8;
const LAVA = 10;
const SAND = 12;
const GRAVEL = 13;
const GOLD_ORE = 14;
const IRON_ORE = 15;
const COAL_ORE = 16;
const WOOD = 17;
const LEAVES = 18;
const SPONGE = 19;
const GLASS = 20;
const LAPIS_ORE = 21;
const LAPIS_BLOCK = 22;
const DISPENSER = 23;
const SANDSTONE = 24;
const NOTE_BLOCK = 25;
const POWERED_RAIL = 27;
const DETECTOR_RAIL = 28;
const STICKY_PISTON = 29;
const COBWEB = 30;
const TALL_GRASS = 31;
const DEAD_BUSH = 32;
const PISTON = 33;
const WOOL = 35;
const DANDELION = 37;
const POPPY = 38;
const BROWN_MUSHROOM = 39;
const RED_MUSHROOM = 40;
const GOLD_BLOCK = 41;
const IRON_BLOCK = 42;
const DOUBLE_STONE_SLAB = 43;
const STONE_SLAB = 44;
const BRICKS = 45;
const TNT = 46;
const BOOKSHELF = 47;
const MOSSY_COBBLESTONE = 48;
const OBSIDIAN = 49;
const TORCH = 50;
const FIRE = 51;
const SPAWNER = 52;
const OAK_STAIRS = 53;
const CHEST = 54;
const REDSTONE_WIRE = 55;
const DIAMOND_ORE = 56;
const DIAMOND_BLOCK = 57;
const CRAFTING_TABLE = 58;
const FARMLAND = 60;
const FURNACE = 61;
const SIGN = 63;
const WOODEN_DOOR = 64;
const LADDER = 65;
const RAIL = 66;
const COBBLESTONE_STAIRS = 67;
const WALL_SIGN = 68;
const LEVER = 69;
const STONE_PRESSURE_PLATE = 70;
const IRON_DOOR = 71;
const WOODEN_PRESSURE_PLATE = 72;
const REDSTONE_ORE = 73;
const REDSTONE_TORCH = 76;
const STONE_BUTTON = 77;
const SNOW = 78;
const ICE = 79;
const SNOW_BLOCK = 80;
const CACTUS = 81;
const CLAY = 82;
const JUKEBOX = 84;
const FENCE = 85;
const PUMPKIN = 86;
const NETHERRACK = 87;
const SOUL_SAND = 88;
const GLOWSTONE = 89;
const JACK_O_LANTERN = 91;
const REDSTONE_REPEATER = 93;
const WHITE_STAINED_GLASS = 95;
const TRAPDOOR = 96;
const STONE_BRICKS = 98;
const ENDER_CHEST = 130;
const DROPPER = 158;
const ENCHANTMENT_TABLE = 116;
const BEACON = 138;
const ANVIL = 145;
const HOPPER = 154;
const COMPARATOR = 404;
const DAYLIGHT_SENSOR = 151;
const QUARTZ = 155;
const IRON_SHOVEL = 256;
const IRON_PICKAXE = 257;
const IRON_AXE = 258;
const FLINT_AND_STEEL = 259;
const APPLE = 260;
const BOW = 261;
const ARROW = 262;
const COAL = 263;
const DIAMOND = 264;
const IRON_INGOT = 265;
const GOLD_INGOT = 266;
const IRON_SWORD = 267;
const WOODEN_SWORD = 268;
const WOODEN_SHOVEL = 269;
const WOODEN_PICKAXE = 270;
const WOODEN_AXE = 271;
const STONE_SWORD = 272;
const STONE_SHOVEL = 273;
const STONE_PICKAXE = 274;
const STONE_AXE = 275;
const DIAMOND_SWORD = 276;
const DIAMOND_SHOVEL = 277;
const DIAMOND_PICKAXE = 278;
const DIAMOND_AXE = 279;
const STICK = 280;
const BOWL = 281;
const MUSHROOM_STEW = 282;
const GOLDEN_SWORD = 283;
const GOLDEN_SHOVEL = 284;
const GOLDEN_PICKAXE = 285;
const GOLDEN_AXE = 286;
const STRING = 287;
const FEATHER = 288;
const GUNPOWDER = 289;
const WOODEN_HOE = 290;
const STONE_HOE = 291;
const IRON_HOE = 292;
const DIAMOND_HOE = 293;
const GOLDEN_HOE = 294;
const SEEDS = 295;
const WHEAT = 296;
const BREAD = 297;
const LEATHER_HELMET = 298;
const LEATHER_CHESTPLATE = 299;
const LEATHER_LEGGINGS = 300;
const LEATHER_BOOTS = 301;
const CHAINMAIL_HELMET = 302;
const CHAINMAIL_CHESTPLATE = 303;
const CHAINMAIL_LEGGINGS = 304;
const CHAINMAIL_BOOTS = 305;
const IRON_HELMET = 306;
const IRON_CHESTPLATE = 307;
const IRON_LEGGINGS = 308;
const IRON_BOOTS = 309;
const DIAMOND_HELMET = 310;
const DIAMOND_CHESTPLATE = 311;
const DIAMOND_LEGGINGS = 312;
const DIAMOND_BOOTS = 313;
const GOLDEN_HELMET = 314;
const GOLDEN_CHESTPLATE = 315;
const GOLDEN_LEGGINGS = 316;
const GOLDEN_BOOTS = 317;
const FLINT = 318;
const PORKCHOP = 319;
const COOKED_PORKCHOP = 320;
const PAINTING = 321;
const GOLDEN_APPLE = 322;
const BUCKET = 325;
const WATER_BUCKET = 326;
const LAVA_BUCKET = 327;
const MINECART = 328;
const SADDLE = 329;
const REDSTONE = 331;
const SNOWBALL = 332;
const BOAT = 333;
const LEATHER = 334;
const MILK_BUCKET = 335;
const BRICK = 336;
const CLAY_BALL = 337;
const SUGAR_CANE = 338;
const PAPER = 339;
const BOOK = 340;
const SLIME_BALL = 341;
const CHEST_MINECART = 342;
const FURNACE_MINECART = 343;
const EGG = 344;
const COMPASS = 345;
const FISHING_ROD = 346;
const CLOCK = 347;
const GLOWSTONE_DUST = 348;
const FISH = 349;
const COOKED_FISH = 350;
const DYE = 351;
const BONE = 352;
const SUGAR = 353;
const CAKE = 354;
const BED = 355;
const REPEATER = 356;
const COOKIE = 357;
const MAP = 358;
const SHEARS = 359;
const MELON = 360;
const PUMPKIN_SEEDS = 361;
const MELON_SEEDS = 362;
const RAW_BEEF = 363;
const STEAK = 364;
const RAW_CHICKEN = 365;
const COOKED_CHICKEN = 366;
const ROTTEN_FLESH = 367;
const ENDER_PEARL = 368;
const BLAZE_ROD = 369;
const GHAST_TEAR = 370;
const GOLD_NUGGET = 371;
const NETHER_WART = 372;
const POTION = 373;
const GLASS_BOTTLE = 374;
const SPIDER_EYE = 375;
const FERMENTED_SPIDER_EYE = 376;
const BLAZE_POWDER = 377;
const MAGMA_CREAM = 378;
const BREWING_STAND = 379;
const CAULDRON = 380;
const EYE_OF_ENDER = 381;
const GLISTERING_MELON = 382;
const SPAWN_EGG = 383;
const BOTTLE_O_ENCHANTING = 384;
const FIRE_CHARGE = 385;
const BOOK_AND_QUILL = 386;
const WRITTEN_BOOK = 387;
const EMERALD = 388;
const ITEM_FRAME = 389;
const FLOWER_POT = 390;
const CARROT = 391;
const POTATO = 392;
const BAKED_POTATO = 393;
const POISONOUS_POTATO = 394;
const EMPTY_MAP = 395;
const GOLDEN_CARROT = 396;
const MOB_HEAD = 397;
const CARROT_ON_A_STICK = 398;
const NETHER_STAR = 399;
const PUMPKIN_PIE = 400;
const FIREWORK_ROCKET = 401;
const FIREWORK_STAR = 402;
const ENCHANTED_BOOK = 403;
const REDSTONE_COMPARATOR = 404;
const NETHER_BRICK = 405;
const NETHER_QUARTZ = 406;
const TNT_MINECART = 407;
const HOPPER_MINECART = 408;
const PRISMARINE_SHARD = 409;
const PRISMARINE_CRYSTALS = 410;
const RABBIT = 411;
const COOKED_RABBIT = 412;
const RABBIT_STEW = 413;
const RABBIT_FOOT = 414;
const RABBIT_HIDE = 415;
const ARMOR_STAND = 416;
const IRON_HORSE_ARMOR = 417;
const GOLDEN_HORSE_ARMOR = 418;
const DIAMOND_HORSE_ARMOR = 419;
const LEAD = 420;
const NAME_TAG = 421;
const COMMAND_BLOCK_MINECART = 422;
const MUTTON = 423;
const COOKED_MUTTON = 424;
const BANNER = 425;
const END_CRYSTAL = 426;
const SPRUCE_DOOR = 427;
const BIRCH_DOOR = 428;
const JUNGLE_DOOR = 429;
const ACACIA_DOOR = 430;
const DARK_OAK_DOOR = 431;
const CHORUS_FRUIT = 432;
const POPPED_CHORUS_FRUIT = 433;
const BEETROOT = 434;
const BEETROOT_SEEDS = 435;
const BEETROOT_SOUP = 436;
const DRAGON_BREATH = 437;
const SPLASH_POTION = 438;
const SPECTRAL_ARROW = 439;
const TIPPED_ARROW = 440;
const LINGERING_POTION = 441;
const SHIELD = 442;
const ELYTRA = 443;
const SPRUCE_BOAT = 444;
const BIRCH_BOAT = 445;
const JUNGLE_BOAT = 446;
const ACACIA_BOAT = 447;
const DARK_OAK_BOAT = 448;
const TOTEM_OF_UNDYING = 449;
const SHULKER_SHELL = 450;
const IRON_NUGGET = 452;
const KNOWLEDGE_BOOK = 453;
const DEBUG_STICK = 455;
const MUSIC_DISC_13 = 2256;
const MUSIC_DISC_CAT = 2257;
const MUSIC_DISC_BLOCKS = 2258;
const MUSIC_DISC_CHIRP = 2259;
const MUSIC_DISC_FAR = 2260;
const MUSIC_DISC_MALL = 2261;
const MUSIC_DISC_MELLOHI = 2262;
const MUSIC_DISC_STAL = 2263;
const MUSIC_DISC_STRAD = 2264;
const MUSIC_DISC_WARD = 2265;
const MUSIC_DISC_11 = 2266;
const MUSIC_DISC_WAIT = 2267;

// items names ig
const ITEM_NAMES = {
  1: "Stone",
  2: "Grass",
  3: "Dirt",
  4: "Cobblestone",
  5: "Wood Plank",
  6: "Sapling",
  7: "Bedrock",
  8: "Water",
  10: "Lava",
  12: "Sand",
  13: "Gravel",
  14: "Gold Ore",
  15: "Iron Ore",
  16: "Coal Ore",
  17: "Wood",
  18: "Leaves",
  19: "Sponge",
  20: "Glass",
  21: "Lapis Ore",
  22: "Lapis Block",
  23: "Dispenser",
  24: "Sandstone",
  25: "Note Block",
  27: "Powered Rail",
  28: "Detector Rail",
  29: "Sticky Piston",
  30: "Cobweb",
  31: "Tall Grass",
  32: "Dead Bush",
  33: "Piston",
  35: "Wool",
  37: "Dandelion",
  38: "Poppy",
  39: "Brown Mushroom",
  40: "Red Mushroom",
  41: "Gold Block",
  42: "Iron Block",
  43: "Double Stone Slab",
  44: "Stone Slab",
  45: "Bricks",
  46: "TNT",
  47: "Bookshelf",
  48: "Mossy Cobblestone",
  49: "Obsidian",
  50: "Torch",
  51: "Fire",
  52: "Spawner",
  53: "Oak Stairs",
  54: "Chest",
  55: "Redstone Wire",
  56: "Diamond Ore",
  57: "Diamond Block",
  58: "Crafting Table",
  60: "Farmland",
  61: "Furnace",
  63: "Sign",
  64: "Wooden Door",
  65: "Ladder",
  66: "Rail",
  67: "Cobblestone Stairs",
  68: "Wall Sign",
  69: "Lever",
  70: "Stone Pressure Plate",
  71: "Iron Door",
  72: "Wooden Pressure Plate",
  73: "Redstone Ore",
  76: "Redstone Torch",
  77: "Stone Button",
  78: "Snow",
  79: "Ice",
  80: "Snow Block",
  81: "Cactus",
  82: "Clay",
  84: "Jukebox",
  85: "Fence",
  86: "Pumpkin",
  87: "Netherrack",
  88: "Soul Sand",
  89: "Glowstone",
  91: "Jack O'Lantern",
  93: "Redstone Repeater",
  95: "White Stained Glass",
  96: "Trapdoor",
  98: "Stone Bricks",
  116: "Enchantment Table",
  130: "Ender Chest",
  138: "Beacon",
  145: "Anvil",
  158: "Dropper",
  154: "Hopper",
  256: "Iron Shovel",
  257: "Iron Pickaxe",
  258: "Iron Axe",
  259: "Flint and Steel",
  260: "Apple",
  261: "Bow",
  262: "Arrow",
  263: "Coal",
  264: "Diamond",
  265: "Iron Ingot",
  266: "Gold Ingot",
  267: "Iron Sword",
  268: "Wooden Sword",
  269: "Wooden Shovel",
  270: "Wooden Pickaxe",
  271: "Wooden Axe",
  272: "Stone Sword",
  273: "Stone Shovel",
  274: "Stone Pickaxe",
  275: "Stone Axe",
  276: "Diamond Sword",
  277: "Diamond Shovel",
  278: "Diamond Pickaxe",
  279: "Diamond Axe",
  280: "Stick",
  281: "Bowl",
  282: "Mushroom Stew",
  283: "Golden Sword",
  284: "Golden Shovel",
  285: "Golden Pickaxe",
  286: "Golden Axe",
  287: "String",
  288: "Feather",
  289: "Gunpowder",
  290: "Wooden Hoe",
  291: "Stone Hoe",
  292: "Iron Hoe",
  293: "Diamond Hoe",
  294: "Golden Hoe",
  295: "Seeds",
  296: "Wheat",
  297: "Bread",
  298: "Leather Helmet",
  299: "Leather Chestplate",
  300: "Leather Leggings",
  301: "Leather Boots",
  302: "Chainmail Helmet",
  303: "Chainmail Chestplate",
  304: "Chainmail Leggings",
  305: "Chainmail Boots",
  306: "Iron Helmet",
  307: "Iron Chestplate",
  308: "Iron Leggings",
  309: "Iron Boots",
  310: "Diamond Helmet",
  311: "Diamond Chestplate",
  312: "Diamond Leggings",
  313: "Diamond Boots",
  314: "Golden Helmet",
  315: "Golden Chestplate",
  316: "Golden Leggings",
  317: "Golden Boots",
  318: "Flint",
  319: "Porkchop",
  320: "Cooked Porkchop",
  321: "Painting",
  322: "Golden Apple",
  325: "Bucket",
  326: "Water Bucket",
  327: "Lava Bucket",
  328: "Minecart",
  329: "Saddle",
  331: "Redstone",
  332: "Snowball",
  333: "Boat",
  334: "Leather",
  335: "Milk Bucket",
  336: "Brick",
  337: "Clay Ball",
  338: "Sugar Cane",
  339: "Paper",
  340: "Book",
  341: "Slime Ball",
  342: "Chest Minecart",
  343: "Furnace Minecart",
  344: "Egg",
  345: "Compass",
  346: "Fishing Rod",
  347: "Clock",
  348: "Glowstone Dust",
  349: "Fish",
  350: "Cooked Fish",
  351: "Dye",
  352: "Bone",
  353: "Sugar",
  354: "Cake",
  355: "Bed",
  356: "Repeater",
  357: "Cookie",
  358: "Map",
  359: "Shears",
  360: "Melon",
  361: "Pumpkin Seeds",
  362: "Melon Seeds",
  363: "Raw Beef",
  364: "Steak",
  365: "Raw Chicken",
  366: "Cooked Chicken",
  367: "Rotten Flesh",
  368: "Ender Pearl",
  369: "Blaze Rod",
  370: "Ghast Tear",
  371: "Gold Nugget",
  372: "Nether Wart",
  373: "Potion",
  374: "Glass Bottle",
  375: "Spider Eye",
  376: "Fermented Spider Eye",
  377: "Blaze Powder",
  378: "Magma Cream",
  379: "Brewing Stand",
  380: "Cauldron",
  381: "Eye of Ender",
  382: "Glistering Melon",
  383: "Spawn Egg",
  384: "Bottle o' Enchanting",
  385: "Fire Charge",
  386: "Book and Quill",
  387: "Written Book",
  388: "Emerald",
  389: "Item Frame",
  390: "Flower Pot",
  391: "Carrot",
  392: "Potato",
  393: "Baked Potato",
  394: "Poisonous Potato",
  395: "Empty Map",
  396: "Golden Carrot",
  397: "Mob Head",
  398: "Carrot on a Stick",
  399: "Nether Star",
  400: "Pumpkin Pie",
  401: "Firework Rocket",
  402: "Firework Star",
  403: "Enchanted Book",
  404: "Redstone Comparator",
  405: "Nether Brick",
  406: "Nether Quartz",
  407: "TNT Minecart",
  408: "Hopper Minecart",
  409: "Prismarine Shard",
  410: "Prismarine Crystals",
  411: "Rabbit",
  412: "Cooked Rabbit",
  413: "Rabbit Stew",
  414: "Rabbit Foot",
  415: "Rabbit Hide",
  416: "Armor Stand",
  417: "Iron Horse Armor",
  418: "Golden Horse Armor",
  419: "Diamond Horse Armor",
  420: "Lead",
  421: "Name Tag",
  422: "Command Block Minecart",
  423: "Mutton",
  424: "Cooked Mutton",
  425: "Banner",
  426: "End Crystal",
  427: "Spruce Door",
  428: "Birch Door",
  429: "Jungle Door",
  430: "Acacia Door",
  431: "Dark Oak Door",
  432: "Chorus Fruit",
  433: "Popped Chorus Fruit",
  434: "Beetroot",
  435: "Beetroot Seeds",
  436: "Beetroot Soup",
  437: "Dragon Breath",
  438: "Splash Potion",
  439: "Spectral Arrow",
  440: "Tipped Arrow",
  441: "Lingering Potion",
  442: "Shield",
  443: "Elytra",
  444: "Spruce Boat",
  445: "Birch Boat",
  446: "Jungle Boat",
  447: "Acacia Boat",
  448: "Dark Oak Boat",
  449: "Totem of Undying",
  450: "Shulker Shell",
  452: "Iron Nugget",
  453: "Knowledge Book",
  455: "Debug Stick",
  2256: "Music Disc 13",
  2257: "Music Disc Cat",
  2258: "Music Disc Blocks",
  2259: "Music Disc Chirp",
  2260: "Music Disc Far",
  2261: "Music Disc Mall",
  2262: "Music Disc Mellohi",
  2263: "Music Disc Stal",
  2264: "Music Disc Strad",
  2265: "Music Disc Ward",
  2266: "Music Disc 11",
  2267: "Music Disc Wait"
};

const RECIPES = [];

// recipes
RECIPES.push(["Wood Planks", WOOD_PLANK, [0, 0, 0, 0, WOOD, 0, 0, 0, 0]]);
RECIPES.push(["Stick", STICK, [0, 0, 0, WOOD_PLANK, WOOD_PLANK, 0, 0, 0, 0]]);
RECIPES.push(["Torch", TORCH, [0, COAL, 0, STICK, 0, 0, 0, 0, 0]]);
RECIPES.push(["Crafting Table", CRAFTING_TABLE, [0, 0, 0, WOOD_PLANK, WOOD_PLANK, 0, WOOD_PLANK, WOOD_PLANK, 0]]);
RECIPES.push(["Furnace", FURNACE, [COBBLESTONE, COBBLESTONE, COBBLESTONE, COBBLESTONE, 0, COBBLESTONE, COBBLESTONE, COBBLESTONE, COBBLESTONE]]);
RECIPES.push(["Bread", BREAD, [0, 0, 0, WHEAT, WHEAT, WHEAT, 0, 0, 0]]);
RECIPES.push(["Ladder", LADDER, [0, STICK, 0, STICK, WOOD_PLANK, STICK, 0, STICK, 0]]);
RECIPES.push(["Chest", CHEST, [WOOD_PLANK, WOOD_PLANK, WOOD_PLANK, WOOD_PLANK, 0, WOOD_PLANK, WOOD_PLANK, WOOD_PLANK, WOOD_PLANK]]);
RECIPES.push(["Bow", BOW, [0, STRING, 0, STICK, STICK, STICK, 0, STRING, 0]]);
RECIPES.push(["Arrow", ARROW, [0, FLINT, 0, STICK, 0, FEATHER, 0, 0, 0]]);
RECIPES.push(["Sign", SIGN, [0, WOOD_PLANK, 0, WOOD_PLANK, STICK, WOOD_PLANK, 0, WOOD_PLANK, 0]]);
RECIPES.push(["Rail", RAIL, [IRON_INGOT, 0, IRON_INGOT, STICK, 0, STICK, STICK, 0, STICK]]);
RECIPES.push(["Minecart", MINECART, [IRON_INGOT, IRON_INGOT, IRON_INGOT, IRON_INGOT, 0, 0, 0, 0, 0]]);
RECIPES.push(["Iron Sword", IRON_SWORD, [0, IRON_INGOT, 0, 0, IRON_INGOT, 0, 0, STICK, 0]]);
RECIPES.push(["Wooden Sword", WOODEN_SWORD, [0, WOOD_PLANK, 0, 0, WOOD_PLANK, 0, 0, STICK, 0]]);
RECIPES.push(["Golden Sword", GOLDEN_SWORD, [0, GOLD_INGOT, 0, 0, GOLD_INGOT, 0, 0, STICK, 0]]);
RECIPES.push(["Diamond Sword", DIAMOND_SWORD, [0, DIAMOND, 0, 0, DIAMOND, 0, 0, STICK, 0]]);
RECIPES.push(["Flint and Steel", FLINT_AND_STEEL, [IRON_INGOT, 0, 0, 0, FLINT, 0, 0, 0, 0]]);
RECIPES.push(["Fishing Rod", FISHING_ROD, [0, STRING, 0, STICK, STICK, 0, STRING, 0, 0]]);
RECIPES.push(["Shears", SHEARS, [IRON_INGOT, IRON_INGOT, 0, 0, 0, 0, 0, 0, 0]]);
RECIPES.push(["Lever", LEVER, [COBBLESTONE, 0, 0, STICK, 0, 0, 0, 0, 0]]);
RECIPES.push(["Redstone Torch", REDSTONE_TORCH, [0, 0, 0, 0, REDSTONE, 0, 0, STICK, 0]]);
RECIPES.push(["Stone Sword", STONE_SWORD, [0, COBBLESTONE, 0, 0, COBBLESTONE, 0, 0, STICK, 0]]);
RECIPES.push(["Stone Pickaxe", STONE_PICKAXE, [COBBLESTONE, COBBLESTONE, COBBLESTONE, 0, STICK, 0, 0, STICK, 0]]);
RECIPES.push(["Stone Axe", STONE_AXE, [COBBLESTONE, COBBLESTONE, 0, COBBLESTONE, STICK, 0, 0, STICK, 0]]);
RECIPES.push(["Iron Shovel", IRON_SHOVEL, [0, IRON_INGOT, 0, 0, STICK, 0, 0, STICK, 0]]);
RECIPES.push(["Iron Pickaxe", IRON_PICKAXE, [IRON_INGOT, IRON_INGOT, IRON_INGOT, 0, STICK, 0, 0, STICK, 0]]);
RECIPES.push(["Iron Axe", IRON_AXE, [IRON_INGOT, IRON_INGOT, 0, IRON_INGOT, STICK, 0, 0, STICK, 0]]);
RECIPES.push(["Wooden Shovel", WOODEN_SHOVEL, [0, WOOD_PLANK, 0, 0, STICK, 0, 0, STICK, 0]]);
RECIPES.push(["Wooden Pickaxe", WOODEN_PICKAXE, [WOOD_PLANK, WOOD_PLANK, WOOD_PLANK, 0, STICK, 0, 0, STICK, 0]]);
RECIPES.push(["Wooden Axe", WOODEN_AXE, [WOOD_PLANK, WOOD_PLANK, 0, WOOD_PLANK, STICK, 0, 0, STICK, 0]]);
RECIPES.push(["Diamond Shovel", DIAMOND_SHOVEL, [0, DIAMOND, 0, 0, STICK, 0, 0, STICK, 0]]);
RECIPES.push(["Diamond Pickaxe", DIAMOND_PICKAXE, [DIAMOND, DIAMOND, DIAMOND, 0, STICK, 0, 0, STICK, 0]]);
RECIPES.push(["Diamond Axe", DIAMOND_AXE, [DIAMOND, DIAMOND, 0, DIAMOND, STICK, 0, 0, STICK, 0]]);
RECIPES.push(["Golden Shovel", GOLDEN_SHOVEL, [0, GOLD_INGOT, 0, 0, STICK, 0, 0, STICK, 0]]);
RECIPES.push(["Golden Pickaxe", GOLDEN_PICKAXE, [GOLD_INGOT, GOLD_INGOT, GOLD_INGOT, 0, STICK, 0, 0, STICK, 0]]);
RECIPES.push(["Golden Axe", GOLDEN_AXE, [GOLD_INGOT, GOLD_INGOT, 0, GOLD_INGOT, STICK, 0, 0, STICK, 0]]);
RECIPES.push(["Iron Hoe", IRON_HOE, [IRON_INGOT, IRON_INGOT, 0, 0, STICK, 0, 0, STICK, 0]]);
RECIPES.push(["Diamond Hoe", DIAMOND_HOE, [DIAMOND, DIAMOND, 0, 0, STICK, 0, 0, STICK, 0]]);
RECIPES.push(["Golden Hoe", GOLDEN_HOE, [GOLD_INGOT, GOLD_INGOT, 0, 0, STICK, 0, 0, STICK, 0]]);
RECIPES.push(["Wooden Hoe", WOODEN_HOE, [WOOD_PLANK, WOOD_PLANK, 0, 0, STICK, 0, 0, STICK, 0]]);
RECIPES.push(["Stone Hoe", STONE_HOE, [COBBLESTONE, COBBLESTONE, 0, 0, STICK, 0, 0, STICK, 0]]);
RECIPES.push(["Stone Bricks", STONE_BRICKS, [STONE, STONE, 0, STONE, STONE, 0, 0, 0, 0]]);
RECIPES.push(["Glass Bottle", GLASS_BOTTLE, [0, GLASS, 0, GLASS, 0, GLASS, 0, 0, 0]]);
RECIPES.push(["Book", BOOK, [0, 0, 0, PAPER, PAPER, PAPER, 0, LEATHER, 0]]);
RECIPES.push(["Paper", PAPER, [0, 0, 0, SUGAR_CANE, SUGAR_CANE, SUGAR_CANE, 0, 0, 0]]);
RECIPES.push(["Bread", BREAD, [0, 0, 0, WHEAT, WHEAT, WHEAT, 0, 0, 0]]);
RECIPES.push(["Golden Apple", GOLDEN_APPLE, [GOLD_INGOT, GOLD_INGOT, GOLD_INGOT, GOLD_INGOT, APPLE, GOLD_INGOT, GOLD_INGOT, GOLD_INGOT, GOLD_INGOT]]);
RECIPES.push(["Iron Door", IRON_DOOR, [IRON_INGOT, IRON_INGOT, 0, IRON_INGOT, IRON_INGOT, 0, IRON_INGOT, IRON_INGOT, 0]]);
RECIPES.push(["Wooden Door", WOODEN_DOOR, [WOOD_PLANK, WOOD_PLANK, 0, WOOD_PLANK, WOOD_PLANK, 0, WOOD_PLANK, WOOD_PLANK, 0]]);
RECIPES.push(["Fence", FENCE, [STICK, WOOD_PLANK, STICK, STICK, WOOD_PLANK, STICK, 0, 0, 0]]);
RECIPES.push(["Ladder", LADDER, [STICK, 0, STICK, STICK, STICK, STICK, STICK, 0, STICK]]);
RECIPES.push(["Sign", SIGN, [WOOD_PLANK, WOOD_PLANK, WOOD_PLANK, WOOD_PLANK, WOOD_PLANK, WOOD_PLANK, 0, STICK, 0]]);
RECIPES.push(["Painting", PAINTING, [STICK, STICK, STICK, STICK, WOOL, STICK, STICK, STICK, STICK]]);
RECIPES.push(["Item Frame", ITEM_FRAME, [STICK, STICK, STICK, STICK, LEATHER, STICK, STICK, STICK, STICK]]);
RECIPES.push(["Chest", CHEST, [WOOD_PLANK, WOOD_PLANK, WOOD_PLANK, WOOD_PLANK, 0, WOOD_PLANK, WOOD_PLANK, WOOD_PLANK, WOOD_PLANK]]);
RECIPES.push(["Crafting Table", CRAFTING_TABLE, [WOOD_PLANK, WOOD_PLANK, 0, WOOD_PLANK, WOOD_PLANK, 0, 0, 0, 0]]);
RECIPES.push(["Furnace", FURNACE, [COBBLESTONE, COBBLESTONE, COBBLESTONE, COBBLESTONE, 0, COBBLESTONE, COBBLESTONE, COBBLESTONE, COBBLESTONE]]);
RECIPES.push(["Anvil", ANVIL, [IRON_BLOCK, IRON_BLOCK, IRON_BLOCK, 0, IRON_INGOT, 0, IRON_INGOT, IRON_INGOT, IRON_INGOT]]);
RECIPES.push(["Enchantment Table", ENCHANTMENT_TABLE, [0, BOOK, 0, DIAMOND, OBSIDIAN, DIAMOND, OBSIDIAN, OBSIDIAN, OBSIDIAN]]);
RECIPES.push(["Ender Chest", ENDER_CHEST, [OBSIDIAN, OBSIDIAN, OBSIDIAN, OBSIDIAN, EYE_OF_ENDER, OBSIDIAN, OBSIDIAN, OBSIDIAN, OBSIDIAN]]);
RECIPES.push(["Beacon", BEACON, [GLASS, GLASS, GLASS, OBSIDIAN, NETHER_STAR, OBSIDIAN, OBSIDIAN, OBSIDIAN, OBSIDIAN]]);
RECIPES.push(["Jukebox", JUKEBOX, [WOOD_PLANK, WOOD_PLANK, WOOD_PLANK, WOOD_PLANK, DIAMOND, WOOD_PLANK, WOOD_PLANK, WOOD_PLANK, WOOD_PLANK]]);
RECIPES.push(["Note Block", NOTE_BLOCK, [WOOD_PLANK, WOOD_PLANK, WOOD_PLANK, WOOD_PLANK, REDSTONE, WOOD_PLANK, WOOD_PLANK, WOOD_PLANK, WOOD_PLANK]]);
RECIPES.push(["Dispenser", DISPENSER, [COBBLESTONE, COBBLESTONE, COBBLESTONE, COBBLESTONE, BOW, COBBLESTONE, COBBLESTONE, REDSTONE, COBBLESTONE]]);
RECIPES.push(["Dropper", DROPPER, [COBBLESTONE, COBBLESTONE, COBBLESTONE, COBBLESTONE, 0, COBBLESTONE, COBBLESTONE, REDSTONE, COBBLESTONE]]);
RECIPES.push(["Hopper", HOPPER, [IRON_INGOT, IRON_INGOT, IRON_INGOT, IRON_INGOT, CHEST, IRON_INGOT, 0, 0, 0]]);
RECIPES.push(["Redston Comparator", REDSTONE_COMPARATOR, [REDSTONE_TORCH, REDSTONE_TORCH, REDSTONE_TORCH, QUARTZ, 0, 0, 0, 0, 0]]);
RECIPES.push(["Repeater", REPEATER, [REDSTONE_TORCH, REDSTONE_TORCH, 0, REDSTONE, 0, 0, 0, 0, 0]]);
RECIPES.push(["Piston", PISTON, [WOOD_PLANK, WOOD_PLANK, WOOD_PLANK, COBBLESTONE, IRON_INGOT, COBBLESTONE, REDSTONE, 0, 0]]);
RECIPES.push(["Sticky Piston", STICKY_PISTON, [PISTON, SLIME_BALL, 0, 0, 0, 0, 0, 0, 0]]);
RECIPES.push(["Lever", LEVER, [COBBLESTONE, STICK, 0, 0, 0, 0, 0, 0, 0]]);
RECIPES.push(["Stone Button", STONE_BUTTON, [STONE, 0, 0, 0, 0, 0, 0, 0, 0]]);
RECIPES.push(["Stone Pressure Plate", STONE_PRESSURE_PLATE, [STONE, STONE, 0, 0, 0, 0, 0, 0, 0]]);
RECIPES.push(["Trapdoor", TRAPDOOR, [WOOD_PLANK, WOOD_PLANK, WOOD_PLANK, WOOD_PLANK, WOOD_PLANK, WOOD_PLANK, 0, 0, 0]]);
RECIPES.push(["Daylight Sensor", DAYLIGHT_SENSOR, [GLASS, GLASS, GLASS, QUARTZ, QUARTZ, QUARTZ, WOOD_PLANK, WOOD_PLANK, WOOD_PLANK]]);RECIPES.push(["TNT", TNT, [SAND, GUNPOWDER, SAND, GUNPOWDER, SAND, GUNPOWDER, SAND, GUNPOWDER, SAND]]);
RECIPES.push(["Fire Charge", FIRE_CHARGE, [GUNPOWDER, BLAZE_POWDER, COAL, 0, 0, 0, 0, 0, 0]]);RECIPES.push(["Written Book", WRITTEN_BOOK, [BOOK_AND_QUILL, 0, 0, 0, 0, 0, 0, 0, 0]]);
RECIPES.push(["Firework Rocket", FIREWORK_ROCKET, [PAPER, GUNPOWDER, 0, 0, 0, 0, 0, 0, 0]]);
RECIPES.push(["Firework Star", FIREWORK_STAR, [GUNPOWDER, DYE, 0, 0, 0, 0, 0, 0, 0]]);
RECIPES.push(["Armor Stand", ARMOR_STAND, [STICK, STICK, STICK, STICK, STONE_SLAB, STICK, 0, 0, 0]]);
RECIPES.push(["Shield", SHIELD, [WOOD_PLANK, WOOD_PLANK, WOOD_PLANK, WOOD_PLANK, IRON_INGOT, WOOD_PLANK, WOOD_PLANK, WOOD_PLANK, WOOD_PLANK]]);
RECIPES.push(["Elytra", ELYTRA, [LEATHER, LEATHER, LEATHER, LEATHER, LEATHER, LEATHER, LEATHER, LEATHER, LEATHER]]);
